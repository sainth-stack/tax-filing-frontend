...
const PieChartComponent = ({ companyDetails, loading }) => {
  ...
  useEffect(() => {
    ...
    const totalCompanies = Object.values(combinationCounts).reduce((sum, value) => sum + value, 0);
    const colors = ["#4318FF", "#A6AEB8", "#C27CFF", "#6AD2FF", "#FB4E22"];

    const data = Object.entries(combinationCounts).map(([name, value], index) => ({
      name,
      value,
      percentage: ((value / totalCompanies) * 100).toFixed(1), // Calculate percentage
  ...
  const onChartClick = (event, index) => {
    if (index !== undefined) {
      const clickedLabel = chartData[index].name;
  ...
  // Custom Label function to display names and percentages outside the pie chart
  const renderLabel = ({ name, percentage, cx, cy, midAngle, outerRadius }) => {
  ...
  return (
    <div className="container">
      <div style={{ width: "100%", height: "400px", borderRadius: "8px", backgroundColor: "#fff", padding: "8px" }}>
        {loading ? (
                    ...
                    <Grid container justifyContent="center" alignItems="center">
                      <PieChart width={550} height={330}>
                        <Pie
                          ...
                          fill="#8884d8"
                          label={renderLabel}
                          onClick={(entry, index, e) => onChartClick(e, index)}
                        >
      ...
      {popupVisible && (
        ...
        >
          <div style={{ textAlign: "center", marginBottom: "12px", fontSize: "18px", fontWeight: "600", color: "#333" }}>
            {clickedLabel}
          </div>
