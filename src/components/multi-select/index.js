...
if (setValue) {
      setValue(selectedOptions); // MultiYearInput usage
    } else if (onChange) {
      onChange(selectedOptions); // Standard React usage
    }
    ...
    return (
      ...
      >
        {/* Move label outside */}
        <Box sx={{ mb: 1, ...labelStyles }}>{label}</Box>
        <StyledSelect
        ...
        >
          ...
          {filteredOptions.length === 0 && (
            <MenuItem disabled>
              {searchText ? "No matching options" : noOptionsMessage}
            </MenuItem>
          )}
        </StyledSelect>
